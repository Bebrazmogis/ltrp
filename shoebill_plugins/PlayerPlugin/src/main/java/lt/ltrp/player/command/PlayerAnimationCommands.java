...
public class PlayerAnimationCommands {
    ...
    public boolean walkStyle(Player p, @CommandParameter(name = "Numeris")int number) {
        ...
        if(number < 0 || number > WalkStyle.values().length)
            ...
            player.sendMessage(getNEWS(Color.Companion), "Vaikðèiojimo stilius paðalintas");
        }
        else {
            player.setWalkStyle(WalkStyle.values()[number-1]);
            player.sendMessage(getNEWS(Color.Companion), "Vaikðèiojimo stilius sëkmingai pakeistas!");
        }
        return true;
    }
    ...
    public boolean talkStyle(Player p, @CommandParameter(name = "Numeris")int number) {
        ...
        if(number < 0|| number > TalkStyle.values().length)
            ...
            player.sendMessage(getNEWS(Color.Companion), "Kalbëjimo stilius paðalintas");
        }
        else {
            player.setTalkStyle(TalkStyle.values()[number-1]);
            player.sendMessage(getNEWS(Color.Companion), "Kalbëjimo stilius sëkmingai pakeistas!");
        }
        return true;
    }
    ...
    public boolean handsUp(Player p) {
        p.setSpecialAction(SpecialAction.HANDSUP);
        return true;
    }
    ...
    public boolean dance(Player p, @CommandParameter(name = "1-13")Integer type) {
        ...
        switch(type) {
            case 1:
                p.setSpecialAction(SpecialAction.DANCE1);
                break;
            case 2:
                p.setSpecialAction(SpecialAction.DANCE2);
                break;
            case 3:
                p.setSpecialAction(SpecialAction.DANCE3);
                break;
            case 4:
                p.setSpecialAction(SpecialAction.DANCE4);
                break;
            case 5:
    ...
    public boolean pee(Player p) {
        p.setSpecialAction(SpecialAction.SPECIAL_ACTION_PISSING);
        return true;
    }
