...
/*
    ...
    public boolean pm(Player player,
        ...
        if(target == null) {
            ...
            target.sendMessage(Color.YELLOW, String.format("(( Gauta PÞ nuo %s[ID:%d]: %s ))", p.getName(), p.getPlayer().getId(), text));
            p.sendMessage(Color.YELLOW, String.format("(( PÞ iðsiûsta %s[ID:%d]: %s ))", target.getName(), target.getPlayer().getId(), text));
