...
(
  ...
  remaining_contract tinyint unsigned not null default '0',
  PRIMARY KEY(id),
  INDEX(player_id),
  INDEX(hours),
  INDEX(job_id),
...
(
  ...
  is_paid tinyint unsigned not null,
  created_at timestamp NOT NULL,
  PRIMARY KEY(id),
  foreign key(player_id) references players(id) on delete cascade,
  foreign key(reported_by) references players(id) on delete cascade
)ENGINE=INNODB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
(
    ...
    foreign key(job_id) references jobs(id) on delete cascade,
    foreign key(player_id) references players(id) on delete cascade,
    foreign key(rank_id) references job_ranks(id) on delete set null
)engine=innodb default charset=utf8 collate=utf8_unicode_ci;
