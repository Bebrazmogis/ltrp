...
import lt.ltrp.dialog.SampModelInputDialog;
...
public final class AdminBusinessManagementDialog {
    ...
    public static ListDialog create(LtrpPlayer player, EventManager eventManager) {
        return ListDialog.create(player, eventManager)
                ...
                }, () -> player.getProperty() == null && Business.getClosest(player.getLocation()) != null, i -> i.getCurrentDialog().show())
                .item(
                        () -> {
                        ...
                        () -> {
                            return (player.getProperty() != null && player.getProperty() instanceof Business)
                ...
                .item("{FF6E62}Kurti naujà verslà", i -> {
                    Business b = Business.create("nëra", player.getLocation(), null, 1, eventManager);
                ...
                .item("Perkelti verslo iðëjimà á mano pozicijà", i -> {
                    showUUIDDialog(player, eventManager, i.getCurrentDialog(), (d, biz) -> {
                        biz.setExit(player.getLocation());
                        eventManager.dispatchEvent(new BusinessEditEvent(biz, player));
                    });
                })
    ...
    private static Business getBusiness(LtrpPlayer p) {
        if(p.getProperty() != null && p.getProperty() instanceof Business)
            return (Business)p.getProperty();
        else
