...
import lt.ltrp.event.PlayerAcceptPlayerQuestion;
...
public class ModeratorCommands {
    ...
    public boolean dcpg(Player p, @CommandParameter(name = "Þaidëjo ID/Dalis vardo")LtrpPlayer target) {
        ...
        if(target == null)
            ...
            player.sendErrorMessage("Ðis þaidëjas nëra uþdavæs klausimo arba jis jau atsakytas.");
        else {
    ...
    public boolean acpq(Player p, @CommandParameter(name = "Þaidëjo ID/Dalis vardo")LtrpPlayer target) {
        LtrpPlayer player = LtrpPlayer.get(p);
        PlayerQuestion question = AdminPlugin.get(AdminPlugin.class).getPlayerQuestion(target);
        if(target == null)
            return false;
    ...
    @Command
    @CommandHelp("Leidþia perþiûrëti paskutinius " + AdminPlugin.QUESTION_LOG_SIZE + " þaidëjø pateiktus klausimus")
