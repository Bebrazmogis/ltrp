...
public class PlayerVehicleCommands extends Commands {
    ...
    public boolean get(Player pp, @CommandParameter(name = "Eilës Numeris")int number) {
        ...
        if(vehicles.length == 0) {
            ...
            if(arrest != null) {
                ...
                player.sendErrorMessage("Negalite iðspawninti daugiau nei " + vehiclePlugin.getMaxOwnedVehicles(player) + " transporto priemoniø.");
            } else {
    ...
    public boolean buyPark(Player p) {
        ...
        if(vehicle == null || !vehicle.getPermissions(player.getUUID()).contains(PlayerVehiclePermission.SetParkingSpace)) {
            ...
            player.sendErrorMessage("Jums neuþtenka pinigø. Parkavimo vietos kaina " + Currency.SYMBOL + vehiclePlugin.getParkingSpaceCost(vehicle.getLocation()));
        } else {
    ...
    public boolean register(Player p) {
        ...
        if(vehicle == null) {
            ...
            player.sendErrorMessage("Jums neuþtenka pinigø. Numeriø kaina " + Currency.SYMBOL + vehiclePlugin.getLicensePrice());
        } else {
            player.giveMoney(-vehiclePlugin.getLicensePrice());
            vehiclePlugin.setLicensePlate(vehicle);
            player.sendMessage(Color.NEWS, "Automobilis áregistruotas. Registracijos kaina " + Currency.SYMBOL + vehiclePlugin.getLicensePrice() + ". Automobilio numeriai: " + vehicle.getLicense());
        }
        return true;
    }
    ...
    @Command
    @CommandHelp("Parodo þaidëjui automobilio baudø istorijà")
    public boolean fines(Player p, @CommandParameter(name = "Þaidëjo ID/ Dalis vardo")LtrpPlayer target) {
        LtrpPlayer player = LtrpPlayer.get(p);
        PlayerVehicle vehicle = PlayerVehicle.getByVehicle(player.getVehicle());
        if(target == null)
            return false;
        if(vehicle == null)
            player.sendErrorMessage("Ðià komandà galite naudoti tik bûdamas savo transporto priemonës viduje");
        else if(vehicle.getOwnerId() != player.getUUID())
            player.sendErrorMessage("Ðià komandà gali naudoti tik transporto priemonës savininkas");
        else if(player.getDistanceToPlayer(target) > 8f)
            player.sendErrorMessage(target.getName() + " yra per toli kad jam parodytumëte savo baudø istorijà");
        else {
    ...
    @Command
    @CommandHelp("Leidþia sumokëti baudas")
    public boolean payFine(Player p) {
        ...
        if(vehicle == null)
            ...
            if(fines.size() == 0)
                player.sendErrorMessage("Jûs neesate gaves baudos arba visas jas sumokëjote! Baudø istorijà galite perþiûrëti su /v fines");
                ...
                VehicleFineListDialog.create(player, eventManager, vehicle, fines)
                        ...
                        .caption("Pasirinkite baudà kurià norite sumokëti")
                        .build()
                        .show();
            }
